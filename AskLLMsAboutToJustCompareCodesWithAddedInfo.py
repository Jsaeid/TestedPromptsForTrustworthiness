...
async def chat():  
    prompt1 = """
Task: Function Criticality Assessment

Context:
In production software, vulnerabilities often emerge in complex and lengthy code units, requiring developers to modify the source code. To enhance software security and maintainability, it is essential to prioritize functions based on their potential risks early in development.

Key Software Metrics for evaluation:

Complexity – Measures the function’s structural complexity (e.g., cyclomatic complexity).
Volume – Represents function size (e.g., lines of code, number of statements).
Coupling – Indicates dependencies and interactions with other functions.
Cohesion – Reflects how well the function's internal components relate to its main purpose.

Objective:
Compare the two provided C functions and assess their criticality regarding maintainability and security risks.

Assign each function to one of the following priority groups:
Highest – Difficult to understand, highly prone to faults or vulnerabilities.
High – Complex but understandable with effort.
Medium – Understandable but could be improved with refactoring.
Low – Easy to understand with minimal risk.
Lowest – Does not require maintenance.
Determine which function is more critical, or if they are equally critical.
Input Format:        
        """  
...
Output Format:
Return only the result in XML format, without explanations or additional text.
    ...
    for i in range(len(modelNames)):  
        for j in range(len(funcsArray)-1): 
            for k in range(j+1, len(funcsArray)):  
                baseFolderName = "Results/AskLLMsAboutToJustCompareCodesWithAddedInfo/"
